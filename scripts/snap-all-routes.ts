...
// Configuración
const BATCH_SIZE = 5; // Número de rutas a procesar en paralelo
const DELAY_BETWEEN_BATCHES = 5000; // Milisegundos de espera entre lotes
...
async function snapToRoad(coordinates: [number, number][]): Promise<[number, number][]> {
  ...
  // Si hay demasiados puntos, dividir en chunks (90 es seguro para Mapbox API)
  if (coordinates.length > 90) {
    ...
    for (const chunk of chunks) {
      try {
        ...
        console.error('Error al procesar chunk de coordenadas:', error);
...
async function processAllRoutes() {
  try {
    ...
    console.log(`Se encontraron ${routes.length} rutas para procesar`);

    // Crear directorio de backup para todas las rutas
    const backupDir = path.join(__dirname, '../data/backup/all_routes');
    await fs.mkdir(backupDir, { recursive: true });
    
    // Hacer un backup completo antes de empezar
    await fs.writeFile(
      path.join(backupDir, `all_routes_backup_${new Date().toISOString().replace(/:/g, '-')}.json`),
      JSON.stringify(routes, null, 2)
    );

    // Dividir las rutas en lotes para no sobrecargar la API
    const routeBatches = chunkArray(routes, BATCH_SIZE);
    ...
    for (let i = 0; i < routeBatches.length; i++) {
      ...
      console.log(`Procesando lote ${i + 1}/${routeBatches.length} (${batch.length} rutas)`);
      
      // Procesar rutas en este lote en paralelo
      const results = await Promise.all(
        batch.map(route => processRoute(route.id))
      );
      ...
      if (i < routeBatches.length - 1) {
        console.log(`Esperando ${DELAY_BETWEEN_BATCHES/1000} segundos antes del siguiente lote...`);
        await new Promise(resolve => setTimeout(resolve, DELAY_BETWEEN_BATCHES));
      }
    }

    console.log(`Proceso completado. Rutas procesadas: ${routes.length}, Éxitos: ${successCount}, Errores: ${errorCount}`);
  } catch (error) {
    console.error('Error al procesar todas las rutas:', error);
  }
}
