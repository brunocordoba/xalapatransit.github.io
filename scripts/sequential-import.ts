...
async function processAllRoutesSequentially() {
  try {
    ...
    // Ordenar rutas por ID para procesarlas en orden
    routeFolders.sort((a, b) => a.id - b.id);
    
    console.log(`Procesando ${routeFolders.length} rutas en orden...`);
    ...
    for (const route of routeFolders) {
      try {
        ...
        // Mostramos estadísticas actuales
    ...
    return { successCount, errorCount, totalStopsCount };
  } catch (error) {
