...
# Usa SQL directo para procesar las rutas con estructura especial
...
for RUTA in $(seq $START_ROUTE $END_ROUTE); do
  ...
  if [ -d "$RUTA_DIR/ruta_1" ]; then
    ...
    node scripts/import-alternativa-directo.js $RUTA 1
  ...
  if [ -d "$RUTA_DIR/ruta_2" ]; then
    ...
    node scripts/import-alternativa-directo.js $RUTA 2
