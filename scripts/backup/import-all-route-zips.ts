...
// Función principal para encontrar e importar todos los route.zip
async function importAllRouteZips() {
  console.log('Iniciando búsqueda de todos los archivos route.zip...');
  ...
  try {
    ...
    console.log(`Encontrados ${routeZips.length} archivos route.zip`);
    
    let successCount = 0;
    let errorCount = 0;
    
    // Procesar cada archivo route.zip
    for (let i = 0; i < routeZips.length; i++) {
      const zipPath = routeZips[i];
      
      try {
        console.log(`\nProcesando archivo ${i+1}/${routeZips.length}: ${zipPath}`);
        ...
        if (route) {
          console.log(`✅ Ruta importada: ${routeName} (ID: ${route.id})`);
          successCount++;
        } else {
...
async function main() {
  try {
    const result = await importAllRouteZips();
