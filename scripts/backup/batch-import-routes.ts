...
// Obtener argumentos de rango de rutas
// Uso: tsx scripts/batch-import-routes.ts 20 30
const args = process.argv.slice(2);
const START_INDEX = args[0] ? parseInt(args[0], 10) : 20;
const END_INDEX = args[1] ? parseInt(args[1], 10) : START_INDEX + 20;
...
async function processRouteRange() {
  console.log(`Iniciando procesamiento de rutas ${START_INDEX}-${END_INDEX}...`);
  ...
  try {
    ...
    console.log(`Encontradas ${routeFolders.length} carpetas de rutas en total`);
    ...
    // Filtrar rutas para el rango especificado
    const filteredRouteInfo = routeInfo.filter(route => 
      route.id >= START_INDEX && route.id <= END_INDEX
    );
    
    // Estadísticas
    console.log(`Procesando ${filteredRouteInfo.length} rutas en el rango ${START_INDEX}-${END_INDEX}`);
    ...
    for (const route of filteredRouteInfo) {
      try {
    ...
    console.log(`\n=== Procesamiento completado del rango ${START_INDEX}-${END_INDEX} ===`);
    ...
    console.error('Error en el procesamiento del rango:', error);
    throw error;
  }
}
...
async function main() {
  try {
    const result = await processRouteRange();
    console.log('Procesamiento de lote finalizado con éxito:');
    console.log(`- ${result.successCount} rutas importadas`);
    console.log(`- ${result.errorCount} errores`);
    console.log(`- ${result.totalStops} paradas creadas`);
    console.log(`\nPara continuar con el siguiente lote, ejecuta:`);
    console.log(`tsx scripts/batch-import-routes.ts ${END_INDEX + 1} ${END_INDEX + 20}`);
  } catch (error) {
