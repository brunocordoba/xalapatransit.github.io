...
// Función principal para generar paradas para todas las rutas excepto la 345
async function generateStopsForAllRoutes() {
  try {
    console.log('Generando paradas para todas las rutas excepto la 345...');
    
    // Obtener todas las rutas excepto la 345 (que ya tiene paradas)
    const routes = await db
      .select()
      .from(busRoutes)
      .where(not(eq(busRoutes.id, 345)))
    ...
    console.log(`Proceso completado. Se generaron ${totalStops} paradas para ${routesWithStops} rutas`);
    return { totalStops, routesWithStops };
  } catch (error) {
    console.error('Error en generateStopsForAllRoutes:', error);
    return { totalStops: 0, routesWithStops: 0 };
  }
}

// Ejecutar la función
generateStopsForAllRoutes()
  .then(({ totalStops, routesWithStops }) => {
    console.log(`Proceso completado con éxito. Se generaron ${totalStops} paradas para ${routesWithStops} rutas.`);
    process.exit(0);
  })
  .catch(error => {
